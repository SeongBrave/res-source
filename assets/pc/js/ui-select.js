...
; + function($) {
    ...
    $.fn.ui_select = function(options) {
        ...
        if (_num === 1) {
			var _data = _this.data(defaults.dataKey);
			if(!_data){
				return new UI_select(_this, options);
			}else{
				_data._setHtml(false);
				return _data;
			}
        };
        ...
        if (_num > 1) {
            _this.each(function(index, el) {
				var _data = $(el).data(defaults.dataKey);
				if(!_data){
					return new UI_select($(el), options);
				}else{
					return _data.refresh();
				}
            })
        }
        // 当元素个数为0时，不执行实例化。
    ...
    UI_select.prototype = {
        ...
        _init: function() {
			...
			this.el.data(defaults.dataKey, this);
        },

        // 组建并获取相关的dom元素;
        _setHtml: function(create) {
			if(create !== false){
				...
				var _w = this._opt.width ? this._opt.width - 0 : this.el.outerWidth() - 0;
				this.el.wrap('<div tabindex="0" class="ui-select-wrap ' + this._opt.wrapClass + '"></div>')
					.after('<div class="ui-select-input"></div><ul class="ui-select-list"></ul>');
			...
			this._wrap = this.el.parent('.ui-select-wrap');
